    ...
    <section className="panel">
      <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "flex-start", flexWrap: "wrap", marginBottom: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(148px, 1fr))", gap: 10, flex: "1 1 620px", maxWidth: 820 }}>
          <div style={{ minWidth: 0, padding: "14px 16px", borderRadius: 18, background: "rgba(192, 192, 192, 0.16)", border: "1px solid rgba(124, 127, 133, 0.18)" }}>
          ...
          {VIEW_OPTIONS.map((option) => (
            <button
              ...
              style={{
                ...
                padding: "14px 16px",
                borderRadius: 18,
                background: selectedView === option.key ? "rgba(124, 127, 133, 0.16)" : "rgba(248, 249, 250, 0.84)",
                border: selectedView === option.key ? "2px solid rgba(73, 80, 87, 0.34)" : "1px solid rgba(124, 127, 133, 0.14)",
                textAlign: "left",
                cursor: "pointer",
        ...
        {selectedView !== "overview" ? (
          <button
            ...
            style={{
              padding: "10px 14px",
              borderRadius: 999,
              border: "1px solid rgba(124, 127, 133, 0.22)",
      ...
      <div style={{ height: 340 }}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 10, right: 12, left: 2, bottom: 0 }}>
            ...
            <Line type="monotone" dataKey="price" name="Price" stroke="#7c7f85" dot={false} strokeWidth={2.5} connectNulls />
            <Line type="monotone" dataKey="regression" name="Trend" stroke="#646d76" dot={false} strokeDasharray="5 5" strokeWidth={1.5} connectNulls />
            {selectedView !== "overview" ? (
              <Line type="monotone" dataKey="projection" name="Projection" stroke="#5c6670" strokeWidth={0} dot={{ r: 5, strokeWidth: 2 }} connectNulls />
            ) : null}
          </LineChart>
        </ResponsiveContainer>
      </div>
